...
const Cart = () => {
  ...
  <div className="container flex justify-around items-start flex-wrap mx-auto mt-8">
    {cartItems.length === 0 ? (<div>Your cart is empty <Link className="font-semibold text-pink-600" to='/shop'>Go to Shop</Link></div>) : (
        <>
            ...
            {cartItems.map((item)=>(
                <div key={item._id} className="flex items-center mb-[1rem] pb-2">
                    <div className="w-[5rem] h-[5rem]">
                        <img src={`${import.meta.env.VITE_API_BASE}/uploads/${item.image}`} alt={item.name} className="w-full h-full object-cover rounded" />
                    </div>
